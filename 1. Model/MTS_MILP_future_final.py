...
def SysCost(model):
    ...
    return oil + gas + slack + fixed + starts + solar + wind + powerflow_cost
...
def MinDown(model,j,i,k):
    if i > 0 and k > i and k < min(i+model.mindn[j]-1,model.HorizonHours):
        ...
        return Constraint.Skip
model.MinimumDown = Constraint(model.Dispatchable,model.HH_periods,model.HH_periods,rule=MinDown)
